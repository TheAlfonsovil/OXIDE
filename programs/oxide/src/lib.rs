...
use anchor_spl::token_2022::{mint_to, burn, Token2022, MintTo, Burn};
use anchor_spl::token_interface::{Mint as MintInterface, TokenAccount as TokenAccountInterface};
use solana_program::program_option::COption;
use solana_program::instruction::AccountMeta;

declare_id!("HExgR2TZiaSLGNBbzHeT6HWGM2NDjQ7f5vjHtV7u94oR");
...
const HOOK_PROGRAM_ID: Pubkey = pubkey!("9dfxKsXLpJZ5Ueerca8rNxByqiywQW2gxhet8CecFm5Q");

// EXPONENTIAL DECAY PARAMETERS
// burn_rate = 20% annual → retention = 80% = 0.8
// Formula: balance_remaining = balance * (0.8)^(elapsed_seconds / SECONDS_PER_YEAR)
//         = balance * (0.8^(1/365))^(elapsed_days) * (1 + ln(0.8) * fraction)
// 
// DAILY_DECAY_FP = 0.8^(1/365) ≈ 0.99938859 in fixed-point (scale 1M) = 999_389
// LN_POINT_EIGHT_FP = ln(0.8) ≈ -0.22314355 in fixed-point (scale 1M) = -223_144
const DAILY_DECAY_FP: u128 = 999_389; // 0.8^(1/365) in fixed-point (scale 1M)
const LN_POINT_EIGHT_FP: i128 = -223_144; // ln(0.8) * 1_000_000 (negative)
const DECAY_SCALE: u128 = 1_000_000; // Fixed-point scale for decay rate
    ...
    pub fn clear_debt(ctx: Context<ClearDebt>) -> Result<()> {
        ...
        if elapsed > 0 {
            ...
            if spl_balance > 0 {
                ...
                // EXPONENTIAL DECAY: balance_remaining = balance * (0.8)^(elapsed_years)
                // burn = balance - remaining

                let remaining = compute_exponential_decay(balance_u128, elapsed as u128);
                let burn_amount = balance_u128.saturating_sub(remaining);
                let burn_u64 = (burn_amount as u64).min(spl_balance);
...
fn sync_tracking_account<'info>(
    ...
    let discriminator: [u8; 8] = anchor_lang::solana_program::hash::hash(b"global:sync_tracking")
        .to_bytes()[..8]
        .try_into()
        .unwrap();
...
/// Compute exponential decay: balance_remaining = balance * (0.8)^(elapsed_seconds / SECONDS_PER_YEAR)
/// 
/// Implementation:
/// 1. Apply daily factor (0.8^(1/365)) for each complete day
/// 2. Apply Taylor approximation for fractional day: (0.8)^x ≈ 1 + x*ln(0.8)
/// 
/// This is HIGHLY PRECISE:
/// - Daily factor is exact (precomputed DAILY_DECAY_FP)
/// - Fractional part uses ln(0.8) ≈ -0.22314355 (more accurate than -0.2)
/// - No exploit possible: decay is continuous and monotonic
fn compute_exponential_decay(balance: u128, elapsed_seconds: u128) -> u128 {
    if elapsed_seconds == 0 || balance == 0 {
        return balance;
    }
    
    let seconds_per_day = SECONDS_PER_DAY as u128;
    
    // Split elapsed time into full days + residual seconds
    let full_days = elapsed_seconds / seconds_per_day;
    let residual_seconds = elapsed_seconds % seconds_per_day;
    
    let mut remaining = balance;
    
    // Apply daily decay factor for each complete day
    // (0.8)^(1/365) per day, iterated full_days times
    for _ in 0..full_days.min(36500) { // Cap at 100 years = 36500 days
    ...
    // Apply fractional day decay using Taylor approximation:
    // (0.8)^(residual/SECONDS_PER_DAY) ≈ 1 + (residual/SECONDS_PER_DAY) * ln(0.8)
    // = 1 + residual * ln(0.8) / SECONDS_PER_DAY
    if residual_seconds > 0 && remaining > 0 {
        // decay_fraction = residual * ln(0.8) / SECONDS_PER_DAY
        // remaining_new = remaining * (1 + decay_fraction)
        //               = remaining + remaining * residual * ln(0.8) / SECONDS_PER_DAY
        
        // Since ln(0.8) is negative, this reduces the balance
        // LN_POINT_EIGHT_FP is already negative, so:
        // delta = (remaining * residual_seconds * LN_POINT_EIGHT_FP) / (DECAY_SCALE * seconds_per_day)
        
        let delta_numerator = (remaining as i128) * (residual_seconds as i128) * LN_POINT_EIGHT_FP;
        let delta_denominator = (DECAY_SCALE as i128) * (seconds_per_day as i128);
        let delta = delta_numerator / delta_denominator; // Negative value
        
        // remaining = remaining + delta (since delta is negative, this decreases remaining)
        if delta < 0 {
            let decrease = (-delta) as u128;
            remaining = remaining.saturating_sub(decrease);
        }
    }
...
fn apply_lazy_burn(user: &mut UserAccount, now: i64) -> Result<()> {
    ...
    if elapsed > 0 && user.balance_free > 0 {
        ...
        // Convert to u64, capped at balance
